<template>
  <div
    v-if="serviceStore.isLoading"
    wire:loading
    class="fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-50 overflow-hidden bg-gray-700 opacity-80 flex flex-col items-center justify-center"
  >
    <div
      class="w-12 h-12 rounded-full animate-spin border-y-8 border-solid border-purple-500 border-t-transparent shadow-md"
    ></div>
    <h2 class="text-center text-white text-xl mt-4">Loading...</h2>
  </div>
</template>
<script setup>
import { useServiceStore } from '../stores/serviceStore.js';
const serviceStore = useServiceStore();
</script>
